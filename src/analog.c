  ...
  struct tm *t = data->t;
	...
	if (data->my_hand == MONTH || data->my_hand == MONTH_TEXT) {
		...
		if (data->my_hand == MONTH) {
			...
			graphics_draw_text(ctx, b_month, fonts_get_system_font(FONT_KEY_GOTHIC_14), temp_grect, GTextOverflowModeWordWrap, GTextAlignmentCenter, NULL);		
		}
	...
	if (data->my_hand == DAY || data->my_hand == DAY_TEXT) {
		...
		c_tm.tm_mday = 0;
		mktime(&c_tm);
		int32_t day_angle = (TRIG_MAX_ANGLE*(t->tm_mday+1)/c_tm.tm_mday);
		...
		if (data->my_hand == DAY) {
			...
			graphics_context_set_text_color(ctx, default_color);
			GRect temp_grect = grect_centered_from_polar(grect_inset(bounds, GEdgeInsets(hand_length/2)), GOvalScaleModeFitCircle, day_angle, GSize(18,18));
			graphics_context_set_fill_color(ctx, GColorWhite);
			graphics_context_set_stroke_color(ctx, GColorDarkGray);
			//graphics_fill_rect(ctx, temp_grect, 1, GCornersAll);
			graphics_draw_round_rect(ctx, temp_grect, 1);
			strftime(b_day, sizeof(char)*3, "%d", t);
			graphics_draw_text(ctx, b_day, fonts_get_system_font(FONT_KEY_GOTHIC_14), temp_grect, GTextOverflowModeWordWrap, GTextAlignmentCenter, NULL);		
		}
	}
	...
	if (data->my_hand == WDAY || data->my_hand == WDAY_TEXT) {
		...
		if (data->my_hand == WDAY) {
			...
			*/
			...
			graphics_context_set_text_color(ctx, default_color);
			GRect temp_grect = grect_centered_from_polar(grect_inset(bounds, GEdgeInsets(hand_length/2)), GOvalScaleModeFitCircle, wday_angle-TRIG_MAX_ANGLE/2/7, GSize(18,18));
			graphics_context_set_fill_color(ctx, GColorWhite);
			graphics_context_set_stroke_color(ctx, GColorDarkGray);
			//graphics_fill_rect(ctx, temp_grect, 1, GCornersAll);
			graphics_draw_round_rect(ctx, temp_grect, 1);
			graphics_draw_text(ctx, b_wday[t->tm_wday], fonts_get_system_font(FONT_KEY_GOTHIC_14), temp_grect, GTextOverflowModeWordWrap, GTextAlignmentCenter, NULL);
		}
	}
